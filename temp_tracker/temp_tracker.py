...
class TempTracker:
    ...
    def _klippy_ready(self):
        self.sensor = self.printer.lookup_object(
            "temperature_sensor " + self.sensor_name
        )
        reactor = self.printer.get_reactor()
        eventtime = reactor.monotonic()
        self.tracker_timer = reactor.register_timer(self.tracker_track, eventtime + 1.0)
    ...
    def get_status(self, eventtime):
        return {"average": self._get_average(), "period": self.period}
    ...
    def query(self, gcmd):
        ...
        gcode.respond_info("Average temp for the past %s seconds: %s" % (secs, average))
