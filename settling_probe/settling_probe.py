...
class SettlingProbeCommandHelper(ProbeCommandHelper):
    def cmd_PROBE_ACCURACY(self, gcmd):
        settling_sample = gcmd.get_int(
            "SETTLING_SAMPLE", self.probe.probe_session.settling_sample
        )
...
class SettlingProbe(PrinterProbe):
    def __init__(self, config):
        ...
        self.cmd_helper = SettlingProbeCommandHelper(
            probe_config, self, self.mcu_probe.query_endstop
        )
        self.probe_offsets = ProbeOffsetsHelper(probe_config)
        self.probe_session = SettlingProbeSessionHelper(
            probe_config, config, self.mcu_probe
        )
